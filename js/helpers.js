...
export function fadeInOutTitle(text, duration = null, delay = null) {
    ...
    // For recordings
    if (new URL(window.location.href).searchParams.get('title') == 'long') {
        delay = 10000
    }

    // Set font size
    Properties.titleText.setFontSize(Constants.TITLE_FONT_SIZE)
    // Update text
    Properties.titleText.setText(text)
    // Fade in and out
    fadeInOut(Properties.titleText, duration, delay)
}
...
export function addGameOverLayer() {
    ...
    let play = Properties.scene.add.image(0, subtitle.y + subtitleSize + offset, 'play')
    // Update origins
...
export function addGameWinLayer() {
    ...
    // Bordered rect
    let width = 1080, height = 472, offsetY = 16
    // Background
    ...
    let title1 = Properties.scene.add.bitmapText(0, titleOffset, 'dark', 'THANKS FOR PLAYING', titleSize)
    let title2 = Properties.scene.add.bitmapText(0, titleOffset + titleSize * 1.3, 'dark', 'AND HAPPY 2021!', titleSize)
    // Donate text and button
    let donateY = title2.y + titleSize + 3 * offsetY, donateSize = 24, btnScale = 0.8
    let lines = [
        'MY NAME IS MAX AND THIS IS MY FIRST GAME',
        'THAT I SPENT 6 MONTHS DEVELOPING ALONE. GAMES ARE',
        'MY PASSION AND NOW I DREAM OF MAKING THEM FULL TIME.',
        'IF YOU LIKED THE GAME, PLEASE CONSIDER DONATING,',
        'SO I CAN FULLY DEVOTE MYSELF TO MAKING GAMES.',
    ]
    let line1 = Properties.scene.add.bitmapText(0, donateY, 'dark', lines[0], donateSize)
    let line2 = Properties.scene.add.bitmapText(0, line1.y + donateSize * 1.4, 'dark', lines[1], donateSize)
